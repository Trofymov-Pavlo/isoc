#!/usr/bin/env bash
# AXIOME - Start (Windows via Git Bash / WSL)
...
# Python: venv Windows prioritaire si présent
if [ -f "$SCRIPT_DIR/Back/venv/Scripts/python.exe" ]; then
  PYTHON_CMD=("$SCRIPT_DIR/Back/venv/Scripts/python.exe")
  echo -e "${GREEN}✓ Utilisation venv Python (Windows)${NC}"
elif command -v python >/dev/null 2>&1; then
  PYTHON_CMD=("python")
  echo -e "${YELLOW}⚠ Utilisation Python système (python)${NC}"
elif command -v py >/dev/null 2>&1; then
  PYTHON_CMD=("py" "-3")
  echo -e "${YELLOW}⚠ Utilisation Python Launcher (py -3)${NC}"
else
  echo -e "${YELLOW}⚠ Python introuvable (installe Python ou crée Back/venv)${NC}" >&2
  exit 1
fi
